...
func (m *LogPackageJoiner) addSafe(lp *_types.LogPackage) {
	m.Lock()
	m.Data[lp.ChunkUid][lp.ChunkI] = lp
	m.Unlock()
}
...
func (m *LogPackageJoiner) Add(lp *_types.LogPackage, tries int) (complete bool, joined *_types.LogPackage) {
	if lp.ChunkI == 0 {
		m.createSafe(lp.ChunkUid, lp.ChunkN, time.Duration(tries)*time.Second)
	} else if m.hasSafe(lp.ChunkUid) == false {
		if tries > 0 {
	...
	m.addSafe(lp)
	return m.completeSafe(lp.ChunkUid)
}
